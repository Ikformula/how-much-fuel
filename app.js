...
const consumptionKml = document.getElementById('consumptionKml');
...
function updateConversions(e) {
  ...
  // 🔄 updated L/km ↔ km/l ↔ mpg conversion
  if (e.target === consumptionLkm && consumptionLkm.value) {
    consumptionKml.value = round(1 / consumptionLkm.value, 4);
    consumptionMpg.value = round(235.215 / (100 * consumptionLkm.value));
  } else if (e.target === consumptionKml && consumptionKml.value) {
    consumptionLkm.value = round(1 / consumptionKml.value, 4);
    consumptionMpg.value = round(235.215 / (100 * consumptionLkm.value));
...
function resetAll() {
  [distanceKm, distanceMiles, priceLiter, priceGallon,
    consumptionLkm, consumptionKml, consumptionMpg].forEach(i => i.value = '');
  currency.value = '₦';
  fuelNeeded.textContent = '-';
  totalCost.textContent = '-';
}
...
[distanceKm, distanceMiles, priceLiter, priceGallon,
  consumptionLkm, consumptionKml, consumptionMpg].forEach(input =>
